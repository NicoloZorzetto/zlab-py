...
def _zform_apply(
    ...
    zforms : Zforms | pandas.DataFrame
        ...
        ['y', 'x', 'Best Transformation', 'Parameters'].
    ...
    try:
        required_cols = {"y", "x", "Best Transformation", "Parameters"}
        ...
        if group_col is None and "Group" in zforms.columns:
            ...
            if auto_group_col is not None:
                ...
                if not silence_warnings:
                    ...
                    ZformApplyWarning(
                        f"No group_col provided — defaulting to original grouping: {group_desc}"
                    )
        ...
        if isinstance(custom_funcs, list):
            if verbose:
                print(
                    "Metadata contains only function names — skipping code restoration."
                )
            custom_funcs = {}
        elif not isinstance(custom_funcs, dict):
            raise TypeError(
                f"Invalid metadata['custom_functions'] type: {type(custom_funcs)}"
            )
        ...
        for name, func_info in custom_funcs.items():
            try:
                ...
                raise RuntimeError(
                    f"Failed to restore custom function '{name}': {e}"
                ) from e
        ...
        def _filter_valid(vars_list, available, name):
            ...
            if not kept:
                raise ValueError(
                    f"No valid {name} remain after filtering (none found in zforms)."
                )
            return kept
        ...
        if subset.empty:
            raise ValueError(
                "No matching transformations found for the given y/x pairs in zforms."
            )
        ...
        def _make_col_name(xv, yv, model, group_name=None):
            ...
            if naming == "standard":
                ...
                raise ValueError(
                    "Invalid naming mode — use 'standard', 'compact', or 'minimal'."
                )
        ...
        if group_col is not None and "Group" in zforms.columns:
            ...
            if verbose and RICH_AVAILABLE:
                console.print(
                    f"Applying group-specific transformations for {len(groups)} groups..."
                )
            ...
            for group_name, gdf in df.groupby("_zgroup"):
                ...
                for _, row in sub_zforms.iterrows():
                    yv, xv, model, params_str = (
                        row["y"],
                        row["x"],
                        row["Best Transformation"],
                        row["Parameters"],
                    )
                    if model == "N/A" or params_str is None:
                        continue
                    if xv not in df.columns or yv not in df.columns:
                        if not silence_warnings:
                            ZformApplyWarning(
                                f"Skipping ({yv}, {xv}) — missing column in df."
                            )
                        continue
                    try:
                        if isinstance(params_str, (list, tuple, np.ndarray)):
                        ...
                        if func is None:
                            raise ValueError(
                                f"Unknown transformation '{model}' — could not apply."
                            )
                        col_name = _make_col_name(xv, yv, model, group_name)
                        mask = df["_zgroup"] == group_name
                        df.loc[mask, col_name] = func(
                            df.loc[mask, xv].to_numpy(), *params
                        )
                    except Exception as e:
                        if not silence_warnings:
                            ZformApplyWarning(
            ...
            for _, row in subset.iterrows():
                ...
                try:
                    ...
                    if func is None:
                        raise ValueError(
                            f"Unknown transformation '{model}' — cannot apply."
                        )
                    ...
                    if not silence_warnings:
                        ZformApplyWarning(
                            f"Failed applying {model} to {xv} ~ {yv}: {e}"
                        )
